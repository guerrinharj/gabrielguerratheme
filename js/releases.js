...
const releasesTag = document.querySelectorAll('[data-info]')
const backTag = document.querySelector('.back')
//Criando uma tag universal
const coverTag = document.querySelectorAll('.release-cover')

//Para cada tag
releasesTag.forEach(tag => {
  //Quando clicamos em uma tag
  tag.addEventListener('click', () => {
    //Aqui retiramos o transform de todas as covers
    coverTag.forEach(cover => {
    ...
    tituloProjects.style.display = 'none'

    menuProjects.forEach(menuli => {
      menuli.classList.add('li-invisible')
    })

    //Aqui aplicamos apenas na cover da tag que clicamos
    tag.querySelector('.release-cover').classList.add('cover-selected')
    ...
    const dataRelease = tag.getAttribute('data-info')



    //Procuramos no document se algum elemento possui o atributo
    const releaseQuery = document.querySelector(dataRelease)

    //Adicionamos a classe nesse elemento que achou o atributo
    releaseQuery.classList.toggle('release-visible')

    backTag.style.opacity = '1'

    backTag.addEventListener('click', () => {
      releaseQuery.classList.remove('release-visible')
